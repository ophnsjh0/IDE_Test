...
**문서 추가/교체 — 웹 (2026-08-10 추가, 권장)**

상단 **Documents** 메뉴에서 처리한다:

- 목록/열람/다운로드: 전 역할 (지식 상세의 "공식 문서 근거"에서 원본 페이지로 바로 이동)
- 업로드(PDF/XLSX): 엔지니어 이상 — 벤더·유형 선택, 같은 이름은 덮어쓰기 선택
- 임베딩 실행(전체/개별)·업로드 자동 임베딩 스위치·문서 삭제: 관리자만

"업로드 자동 임베딩"이 켜져 있으면(기본) 업로드 즉시 그 파일만 임베딩되고,
꺼져 있으면 관리자가 "미임베딩 문서 임베딩" 버튼으로 일괄 처리한다.
동시 실행은 파일 잠금으로 차단(gmail 동기화와 동일 방식).

**문서 추가/교체 — CLI (대량 이관 시)**
```bash
...
주의: docker-compose의 reference_docs 마운트는 웹 업로드를 위해 **쓰기 가능**이어야
한다 (2026-08-10부터 `:ro` 제거됨 — VM 배포 시 compose 반영 필요).
