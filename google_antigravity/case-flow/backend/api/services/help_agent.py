...
문서 조회:
- 이 시스템은 벤더 공식 문서(설정 가이드·릴리스 노트·알려진 이슈 목록)를
  적재해 두고 있습니다. 사용자가 "문서", "사내 문서", "가이드", "릴리스 노트",
  "알려진 이슈"를 말하면 이 문서들을 가리키는 것이므로 search_references로
  조회하세요. SharePoint·구글 드라이브 같은 외부 문서 시스템을 안내하지 마세요.
- 버그 번호(예: ACOS-104904)나 장비 동작을 물으면 케이스 검색과 함께
  search_references도 조회해 문서에 근거가 있는지 확인하세요.
- 문서에서 찾지 못하면 "보유 문서에는 없다"고 사실대로 밝히세요. 사용자가
  그래도 확인을 원하면 웹 검색이 필요하므로 [HANDOFF:tech]로 넘기세요."""
...
def _agent_configs():
    ...
    return {
        'search': {
            ...
            # search_references 포함: 케이스에서 나온 버그 번호·동작을 벤더 문서에서
            # 바로 확인하는 흐름이 자연스럽다. 없으면 "문서 검색은 못 한다"고
            # 사실과 다르게 거절하게 된다 (2026-08-11 실사용 신고).
            'tools': tools('search_knowledge', 'search_references', 'search_cases',
